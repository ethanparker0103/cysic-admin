...
const VerifierDetail = () => {
    ...
    const { data } = useRequest(
        () => {
            ...
            return axios.get(`/api/v1/zkTask/dashboard/verifier/detail/${params?.id}`);
        },
        {
            ready: !!params?.id,
    ...
    const { data: listData, currentPage, total, setCurrentPage } = usePagnation(
        (page: number) => {
            return axios.get(`/api/v1/zkTask/dashboard/verifier/taskList/${params?.id}`, {
                params: {
                    pageNum: page,
    ...
    return (
        <div className={cn("mx-auto mb-auto relative z-10 pt-20 pb-16 w-full", isMobile ? "break-words" : "")}>
            {/* title */}
            <h1 className={cn("title !font-[200] mb-24 text-center", isMobile ? "text-7xl" : "text-[4rem]")}>Verifier Detail</h1>
