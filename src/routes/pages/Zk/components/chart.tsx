...
const Chart = ({ row = [], hasData }: any) => {
    const [leftDomain, rightDomain] = useMemo(() => scaleDomains(
        [Math.min(...row?.map((d) => d.finishTaskCnt)), Math.max(...row?.map((d) => d?.finishTaskCnt))],
        [Math.min(...row?.map((d) => d.newTaskCnt)), Math.max(...row?.map((d) => d?.newTaskCnt))]
    ), [row]);
    ...
    <div className="flex items-center gap-2">
            ...
            <div className={`size-full ${hasData ? '' : 'opacity-60'}`}>
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart
                        data={row?.map((i: { date: string | number | dayjs.Dayjs | Date | null | undefined; finishTaskCnt: string | number; }) => {
                            return ({
                                ...i,
                    ...
                    >
                        ...
                        <XAxis dataKey="date" />
                        <YAxis hide orientation='left' yAxisId='left' dataKey="finishTaskCnt" stroke='#387908' domain={leftDomain} ticks={generateTicks(leftDomain)} />
                        <YAxis hide orientation='right' yAxisId='right' dataKey="newTaskCnt" stroke='#38abc8' domain={rightDomain} ticks={generateTicks(rightDomain)} />
