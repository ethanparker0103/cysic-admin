...
const NewToModal = () => {
    ...
    const { visible, setVisible } = useModalState({eventName: 'modal_new_to_visible'})
                ...
                <div className="w-[440px] p-10 flex flex-col gap-10 bg-[#0B0C0F] relative z-1">
                    ...
                    <div className="flex flex-col gap-4 -mt-3">
                        ...
                        <Upload
                            ...
                            className={clsx('absolute left-1/2 -translate-x-1/2 [&_.arco-upload-trigger-picture]:rounded-full [&_.arco-upload-trigger-picture]:border-gradient', errorShow && !formValue?.logo ? "border border-[#da1a1a]" : "border-[#525252]")
                            }
                        />
                        </div>
                        <FormItem title={"Username"}>
                            <Input value={formValue?.name} onChange={(v: any) => updateFromValue('name', v)} className={clsx((errorShow && !formValue?.name) ? "border-[#da1a1a]" : "border-[#525252]", "rounded-[8px]")} placeholder="name" />
                        </FormItem>

                    </div>
                    <Button needLoading onClick={handleSubmit} type="gradient">Register Account</Button>
