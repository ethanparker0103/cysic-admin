...
const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    if (active && payload && payload.length) {
        ...
        const value = payload[0]?.payload?.finish_task_cnt
        const new_task_cnt = payload[0]?.payload?.new_task_cnt
            ...
            <div className="py-4 px-6 bg-[#0F0F0F] rounded-[24px] border flex flex-col gap-1 shadow-[0px_4px_0px_0px_#000000]">
                <div className='flex flex-col gap-1'>
                    <div className='flex items-center gap-1'>
                        ...
                        <div className='text-[#A3A3A3]'>{t('new_task_cnt')}:</div>
                    </div>
                    <span className='font-bold pl-4'>{new_task_cnt}</span>
                </div>
                <div className='flex flex-col gap-1'>
                    <div className='flex items-center gap-1'>
                        <div className='size-3 rounded-full bg-[#9747FF]' />
                        <div className='text-[#A3A3A3]'>{t('finish_task_cnt')}:</div>
                    </div>
...
const Chart = ({ row = [], hasData }: any) => {
    ...
    const [leftDomain, rightDomain] = useMemo(() => scaleDomains(
        [Math.min(...row?.map((d) => d.finish_task_cnt)), Math.max(...row?.map((d) => d?.finish_task_cnt))],
        [Math.min(...row?.map((d) => d.new_task_cnt)), Math.max(...row?.map((d) => d?.new_task_cnt))]
    ), [row]);
        ...
        <div className='size-full relative'>
            ...
            <div className={`size-full ${hasData ? '' : 'opacity-60'}`}>
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart
                        data={row?.map((i: { date: string | number | dayjs.Dayjs | Date | null | undefined; finish_task_cnt: string | number; }) => {
                            return ({
                                ...i,
                                time: dayjs(i?.date).toDate(),
                                value: +i?.finish_task_cnt
                            })
                        })}
                    ...
                    >
                        ...
                        <XAxis dataKey="date" />
                        <YAxis hide orientation='left' yAxisId='left' dataKey="finish_task_cnt" stroke='#387908' domain={leftDomain} ticks={generateTicks(leftDomain)} />
                        <YAxis hide orientation='right' yAxisId='right' dataKey="new_task_cnt" stroke='#38abc8' domain={rightDomain} ticks={generateTicks(rightDomain)} />

                        <Tooltip content={<CustomTooltip />} />

                        <defs>
                            <linearGradient id="splitColor" x1="0" y1="0" x2="0" y2="1">
                                <stop offset={0} stopColor="var(--accentGreen)" stopOpacity={0.5} />
                                <stop offset={'100%'} stopColor="var(--accentGreen)" stopOpacity={0} />
                            </linearGradient>
                        </defs>
                        <defs>
                            <linearGradient id="splitColor2" x1="0" y1="0" x2="0" y2="1">
                                <stop offset={0} stopColor="var(--accentPurpe)" stopOpacity={0.5} />
                                <stop offset={'100%'} stopColor="var(--accentPurpe)" stopOpacity={0} />
                            </linearGradient>
                        </defs>

                        <Bar yAxisId="right" type="monotone" dataKey="new_task_cnt" barSize={20} fill="#21E9FA" />
                        <Line yAxisId="left" type="monotone" dataKey="finish_task_cnt" stroke="#9747FF" strokeWidth={5} />

                        <Legend align="left" formatter={(str)=>t(str)}/>
